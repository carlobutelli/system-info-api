...
function refreshProcesses() {
   $.ajax({
      ...
      success: function (data) {
         $('#process_tab').empty();
         $('#proc_count').text(data.processes.length);
         for(var p = 1; p < data.processes.length; p++) {
             $('#process_tab').append('<tr><td>'+data.processes[p].pid+'</td>'+
                '<td>'+data.processes[p].name+'</td>'+
                '<td>'+data.processes[p].memory_percent.toFixed(2)+'</td>'+
                '<td>'+(data.processes[p].cpu_times[0]+data.processes[p].cpu_times[1]).toFixed(2)+'</td>'+
                '<td>'+data.processes[p].num_threads+'</td>'+
                '</tr>')
         }
